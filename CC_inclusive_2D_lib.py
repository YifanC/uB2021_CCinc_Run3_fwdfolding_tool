...
overlay_norm = 2.144E20 / 1.268E21
dirt_norm = 2.144E20 / 1.25E20
ext_norm = 51546294. / 85768579.
...
# def histBkg():
#     h_bin_bkg = ROOT.TH1F("h_bin_bkg", "h_bin_bkg", len_theta, 0, len_theta)
#     for i_bin in range(len_theta):
#         if i_bin%2 == 0:
#             h_bin_bkg.SetBinContent(i_bin+1,10000)
#         if i_bin%2 == 1:
#             h_bin_bkg.SetBinContent(i_bin+1,0)
...
def predict(h_true,this_pot):
    ...
    return pred_vec, ext_vec, dirt_vec, bkg_vec, reco_vec, h_pred
...
def return_pot_covar(h_true,this_pot):
    ...
    pot_vec = dirt_vec + bkg_vec + reco_vec
    
    if n_bins == 43:
        n_bins-=1
    for i in range(n_bins):
        for j in range(n_bins):
            frac_covar_pot[i][j] = (pot_vec[i]*0.02)*(pot_vec[j]*0.02)/(pred_vec[i]*pred_vec[j])
...
def eventrate_comparison(h_data_func,h_true_func,this_pot,this_frac_tot, filename, path):
    c1 = ROOT.TCanvas("c1","c1",800,600)
    c1.SetGrid(1)
    ...
    pred_vec, ext_vec, dirt_vec, bkg_vec, reco_vec, h_pred = predict(h_true_func,this_pot)
    ...
    h_reco_func.SetFillColor(ROOT.kRed) # signal
    # h_reco_func.SetLineColor(ROOT.kRed)
    h_ext_func.SetLineWidth(0)
    h_bkg_func.SetLineWidth(0)
    h_dirt_func.SetLineWidth(0)
    h_reco_func.SetLineWidth(0)
    ...
    h_data_func.GetXaxis().SetTitle("Reco bin number")
    h_data_func.GetYaxis().SetTitle("Number of events")
    h_data_func.GetYaxis().SetTitleOffset(0.75)
    h_data_func.GetYaxis().SetTitleSize(0.07)
    ...
    h_tot.SetLineColorAlpha(1,0)
    h_tot.SetFillStyle(3004)
    # h_tot.SetFillStyle(3004)

    h_ext_func.SetLineWidth(2)
    h_data_func.SetLineColor(1)
    ...
    legend = ROOT.TLegend(0.12,0.5,0.48,0.9)
    legend.SetTextSize(0.039)
    legend.AddEntry(h_ext_func,'Beam-off background (cosmic)',"f");
    legend.AddEntry(h_dirt_func,'Dirt background (cosmic + #nu)');
    legend.AddEntry(h_bkg_func,'Beam-on background (cosmic + #nu)'); 
    legend.AddEntry(h_reco_func,'#nu_{#mu} CC');
    legend.AddEntry(h_tot,'Systematic uncertainties '); 
    legend.AddEntry(h_data_func,'Data w/ stat, #chi^{2}/NDF: ' + str(round(chi2ndf,2)),"lep")
      
    # legend.AddEntry('#chi^{2}:'+'{:04.1f})'.format(chi2) ',"f");
    #legend.AddEntry(h_tot,'Syst. error, #chi^{2}:'+'{:04.1f})'.format(chi2),"f");
    legend.Draw()
    
    h_data_func.Draw('PE1 same')
    ...
    h_xsec_data_ratio.SetYTitle('Data/Prediction')
    ...
    h_xsec_data_ratio.Draw('PE1')
    # h_histo.Draw('hist same')
    h_mc_ratio.Draw('E2 same')

    c1.Draw()
    # c1.SaveAs(path + filename+".png")
    # c1.SaveAs(outputdir + filename+".root")
    c1.SaveAs(path + filename+".pdf")

    return

def fakedata_eventrate_comparison(h_overlay_func,h_true_func,this_pot,this_frac_tot, filename, path):
    c1 = ROOT.TCanvas("c1","c1",800,600)
    c1.SetGrid(1)
    c1.SetLeftMargin(0.15)
    c1.SetRightMargin(0.02)
    ...
    h_bkg_func = f_in.Get('h_bkg_cv')
    h_dirt_func = f_in.Get('h_dirt_cv')
    h_ext_func = f_in.Get('h_ext_cv')

    # h_bkg_func.Scale(overlay_norm) # bkg from overlay
    # h_dirt_func.Scale(dirt_norm)
    # h_ext_func.Scale(ext_norm)

    h_bkg_func.Scale(this_pot/2.144e+20) # bkg from overlay
    h_dirt_func.Scale(this_pot/2.144e+20)
    h_ext_func.Scale(this_pot/2.144e+20)
    ...
    # h_overlay_sig_reco_func
    h_reco_func = arr2plot(reco_vec)

    # num_bins = h_true_func.GetNbinsX() # get eventually better than hardcode    
    num_bins = 42 # xsec bins, do not include overflow    
    

    # cc_bla = ROOT.TCanvas()
    # h_true_func.SetLineColor(1)
    # h_reco_func.SetLineColor(2)

    # h_true_func.Draw("hist")
    # h_reco_func.Draw("hist, same")

    # cc_bla.SaveAs("test.pdf")
    
    # pred_vec = reco_vec + dirt_vec+ bkg_vec + ext_vec

    # h_pred = arr2plot(pred_vec) # event rate from the model prediction
    ...
    h_tot = h_reco_func.Clone("h_tot")
    h_tot.Sumw2()
    h_tot.Add(h_bkg_func)
    h_tot.Add(h_dirt_func)
    h_tot.Add(h_ext_func)

    h_data_func = ROOT.TH1F("h_data_func","", num_bins, 0, num_bins)
    for i_bin in range(num_bins):
    # h_data_func = ROOT.TH1F("h_data_func","", 42, 0, 42)
    # for i_bin in range(42):
        h_data_func.SetBinContent(i_bin+1, h_overlay_func.GetBinContent(i_bin+1) + h_ext_func.GetBinContent(i_bin+1) + h_dirt_func.GetBinContent(i_bin+1))
    ...
    covar_data = np.zeros((num_bins,num_bins))
    # covar_data = np.zeros((num_bins-1,num_bins-1))
    for i in range(num_bins):
        for j in range(num_bins):
    # for i in range(num_bins-1):
    ...
    for i in range(num_bins):
        h_tot.SetBinError(i+1,math.sqrt(this_frac_tot[i][i])*h_tot.GetBinContent(i+1))
        # print "bin " + str(i) + " error"
        # print math.sqrt(this_frac_tot[i][i])*h_tot.GetBinContent(i+1)
     

    h_ext_func.SetLineWidth(2)
    h_data_func.SetMarkerStyle(20)
    h_data_func.SetMarkerSize(1)
    h_data_func.SetLineColor(1)
    h_data_func.SetLineWidth(0)
    ...
    legend.AddEntry(h_reco_func,'#nu_{#mu} CC');
    legend.AddEntry(h_tot,'Systematic uncertainties '); 
    legend.AddEntry(h_data_func,'Fake data, #chi^{2}: ' + str(round(chi2,4)),"lep")
    # legend.AddEntry(h_data_func,'Data w/ stat, #chi^{2}/NDF: ' + str(round(chi2ndf,4)),"lep")
      
    # legend.AddEntry('#chi^{2}:'+'{:04.1f})'.format(chi2) ',"f");
    #legend.AddEntry(h_tot,'Syst. error, #chi^{2}:'+'{:04.1f})'.format(chi2),"f");
    legend.Draw()
    
    h_data_func.Draw('P same')
    ...
    h_xsec_data_ratio.SetMarkerStyle(20)
    h_xsec_data_ratio.SetMarkerSize(1)
    h_xsec_data_ratio.Draw('P')
    # h_xsec_data_ratio.Draw('PE1')
    # h_histo.Draw('hist same')
    h_mc_ratio.Draw('E2 same')
    ...
    c1.Draw()
    c1.SaveAs(path + filename+"_alluncertainties.png")
    # c1.SaveAs(outputdir + filename+".root")
    c1.SaveAs(path + filename+"_alluncertainties.pdf")
...
def chi2_minus1(h_data, h_true_func,this_pot, frac):
    ...
    all_vec = predict(h_true_func,this_pot)
    pred_vec = all_vec[0]    
    covar_h = np.zeros((42,42))
    for i in range(42):
        for j in range(42):
            covar_h[i][j] = frac[i][j]*pred_vec[i]*pred_vec[j]
